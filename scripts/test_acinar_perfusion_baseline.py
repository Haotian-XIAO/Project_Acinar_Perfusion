...
def run_Acinar_Perfusion(
    ...
    for cell in cell_metadata:
        ...
        # only keep cells that have computed scores
        if cid not in score_map:
            continue

        verts = cell["vertices"]   # list of [x, y]
        poly = Polygon(verts, closed=True)

        patches.append(poly)
        values.append(score_map[cid])

        centers_x.append(cell["center"][0])
        centers_y.append(cell["center"][1])
    ...
    ax.set_title("Voronoi cell perfusion map")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ...
    # ===== save figure =====
    save_path = "/Users/xiao/PhD/Project_Acinar_Perfusion/results/voronoi_cell_perfusion_map.png"
...
load_params["liquid"]["Theta_out_fin_lst"] = [0.001]
